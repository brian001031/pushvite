...
import { group_ht_aging_record } from "../../../../mes_remak_data";
...
const HTAging = () => {
    ...
    const handle_Introduce_View = () => {
      const sideoption = "cutting_cathode";
      console.log("side option = " + sideoption);
      navigate(`/Mes_WorkflowIntroduce/${sideoption}`);
    };
    useEffect (()=>{
    setMachineOption("高溫靜置倉");
    },[])
  

    // 機台或日期變更時都呼叫 callht_aging
    useEffect(() => {
      if (!machineOption || !startDate) return;
      const fetchQuality = async () => {
          
        try {
          const response = await api.callht_aging(
            machineOption,
            startDate
          );

          console.log("API回傳 callht_aging (品質):", response);
    ...
    const fetchData = async () => {
      try {
          const response = await api?.callht_aging(machineOption , startDate || "");
          console.log("API回傳 callht_aging:", response);
          if (response?.length) {
        ...
        console.error("callht_aging API 錯誤:", error);
        setResponseData({});
      }
    };
    ...
    useEffect(() => {
      ...
      const fetchQuality = async () => {
        try {
          const response = await api?.callht_aging_groupname_capacitynum(
            machineOption || "",
            startDate.format("YYYY-MM-DD") || ""
          );
  
          console.log("API回傳 callht_aging_groupname_capacitynum:", response);
          if (response) {
          ...
          console.error("callht_aging_groupname_capacitynum API 錯誤:", error);
          setLeftData({});
        }
      };
    ...
    const varName = String("group_ht_aging_record").trim();
    ...
    return (
      <div style={{ maxWidth: "100vw", overflowX: "auto" }}>
        ...
        >
          <option value="高溫靜置倉">高溫靜置倉</option>
        </select>
            ...
            <div className="LeftContent">
              <div className="Content_Top">
                ...
                <div className="Answer" style={getColorStyle("BOX_BATT")}>
                  {responseData.BOX_BATT? responseData.BOX_BATT.slice(0, 7) : null || "抓取中"}
                </div>
          ...
          <Col lg={5} md={5} sm={12} style={{ minWidth: 600 }}>
            <div className="MiddleContent">
              <div className="Content_Top">
                ...
                >
                  {Object.keys(group_ht_aging_record).map((groupName) => {
                    const labelMap = group_ht_aging_record[groupName]?.[0] || {};
