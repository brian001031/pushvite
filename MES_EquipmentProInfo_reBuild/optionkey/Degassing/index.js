...
import {group_degassing_2_record , group_degassing_3_record} from "../../../../mes_remak_data";
...
const [responseDataQuality, setResponseDataQuality] = useState({}); // api groupname_htagingnum 資料
...
useEffect(() => {
    ...
    try {
        if (modalIsOpen === true) {
         ...
         const response = await api.calldegassing(
            machineOption,
            startDate
        );

        console.log("API回傳 calldegassing (品質):", response);
...
const fetchData = async () => {
    ...
    try {
        ...
        if (modalIsOpen === true) {
            ...
            const response = await api?.calldegassing(machineOption);
            console.log("API回傳 calldegassing:", response);
            if (response?.length) {
    ...
    console.error("calldegassing API 錯誤:", error);
    setResponseData({});
    }finally{
        setIsMainDataLoading(false);
    }
...
useEffect(() => {
    ...
    previousDataRef.current = { ...responseData, ...leftData };
}, [responseData, leftData , modalIsOpen]);
...
const fetchQuality = async () => {
    ...
    try {
        ...
        if (modalIsOpen === true) {
            ...
            const response = await api?.calldegassing_groupname_capacitynum(
            machineOption || "",
            startDate.format("YYYY-MM-DD") || ""
            );

            console.log("API回傳 calldegassing_groupname_capacitynum:", response);

            let normalized = {};
            if (response?.data && typeof response.data === "object" && !Array.isArray(response.data)) {
            // 主要資料在 data 物件
            normalized = { ...response.data };
            // 如果同時帶有人員資料，補上（支援兩種位置）
            if (response.staffRows) normalized.staffRows = response.staffRows;
            if (response.data?.staffRows) normalized.staffRows = response.data.staffRows;
            } else if (Array.isArray(response) && response.length > 0) {
                normalized = { ...response[0] };
            } else if (response && typeof response === "object") {
                normalized = { ...response };
            }
            setLeftData(normalized);
        }
        
    } catch (error) {
        console.error("calldegassing_groupname_capacitynum API 錯誤:", error);
        setLeftData({});
    }
    };
...
useEffect(() => {
    ...
    try{
        ...
        console.error("fetchQuality 錯誤:", error);
        throw error;
    }finally{
        setIsMainDataLoading(false);
    }
...
const varName = String(machineOption).trim().includes("二抽") ? "group_degassing_2_record" : "group_degassing_3_record" ;
...
useEffect (()=>{
    ...
    if (String(machineOption)?.trim().includes("二抽一期") || String(machineOption)?.trim().includes("二抽二期")) {
        setNowInner(group_degassing_2_record)
    }
    else if (String(machineOption)?.trim().includes("三抽一期") || String(machineOption)?.trim().includes("三抽二期")) {
        setNowInner(group_degassing_3_record)
    }
    else {
        setNowInner({})
    }
...
return (
    ...
    >
        <option value="二抽一期">二抽一期</option>
        <option value="二抽二期">二抽二期</option>
        <option value="三抽一期">三抽一期</option>
        <option value="三抽二期">三抽二期</option>
    </select>
    ...
    <Row className="EdgeFoldingRow" style={{ flexWrap: "nowrap" }}>
        ...
        <div className="LeftContent">
            ...
            <div className="Answer" style={getColorStyle("cellNO")}>
                ...
                responseData.Barcode.slice(0, 7)
                ) : (
                ""
                ) || "抓取中"}{" "}
            </div>
            ...
            <div className="Answer" style={getColorStyle("BOX_BATT")}>
                {responseData.PLCCellID1_CE? responseData.PLCCellID1_CE.slice(0, 7) : null || "抓取中"}
            </div>
            <div className="Content">●當日產能:</div>
            <div
                className="Answer"
                key={"today_capacitynum"}
                style={getColorStyle("today_capacitynum")}
            >
                {leftData?.today_capacitynum || "抓取中"} PCS
            </div>
            ...
            {startDate &&
            leftData?.amount_capacitynum !== null ? (
                <div
                className="Answer"
                ...
                >
                累積產能 :
                {leftData?.amount_capacitynum || "抓取中"} PCS
                </div>
            ) : (
            ...
            {shiftClass === "早班" ? (
                <div
                className="Answer"
                key={"morning_capacitynum"}
                style={getColorStyle("morning_capacitynum")}
                >
                班別產能:
                {leftData?.morning_capacitynum || "抓取中"} PCS
                </div>
            ) : (
                <div
                className="Answer"
                key={"night_capacitynum"}
                style={getColorStyle("night_capacitynum")}
                >
                班別產能:
                {leftData?.night_capacitynum || "抓取中"} PCS
                </div>
            )}
        ...
        <div className="MiddleContent">
            ...
            >
                {Object.keys(nowInner).map((groupName) => {
                const labelMap = nowInner[groupName]?.[0] || {};
