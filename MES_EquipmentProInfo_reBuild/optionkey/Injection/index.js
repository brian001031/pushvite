...
import { group_Inject_01, group_Inject_02 } from "../../../../mes_remak_data";
...
const CuttingCathode = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await api?.callinjection(machineOption || "");
        ...
        console.error("callinjection API 錯誤:", error);
        setResponseData({});
      }finally{
  ...
  useEffect(() => {
    ...
    if (modalIsOpen === true) {
        console.log ("A fetchQuality 被呼叫 machineOption:");
        return;
      }
    ...
    const fetchQuality = async () => {
      ...
      setIsMainDataLoading(true);
      try {
        const response = await api?.callinjection_groupname_capacitynum(
          machineOption || "",
  ...
  let varName = "";
  if (machineOption === "注液機") {
    varName = "group_Inject01";
  } else if (machineOption === "注液機二期") {
    varName = "group_Inject02";
  }
  ...
  useEffect(() => {
    ...
    try {
      ...
      const fetchReference = async () => {
        if (modalIsOpen === true) {
              console.log ("B fetchReference 被呼叫 machineOption:");
        return;
      }
  ...
  return (
    <div style={{ maxWidth: "100vw", overflowX: "auto" }}>
      ...
      >
        <option value="注液機">注液機</option>
        <option value="注液機二期">注液機二期</option>
      </select>
            ...
            <div className="Content_Top">
              ...
              <div className="Answer">
                ...
                >
                  {shiftClass === "早班" ? (
                    <div>
                      {leftData?.staffRows?.[0]?.OPNO || "抓取中"}|{" "}
                      {leftData?.staffRows?.[0]?.StaffName1 || "抓取中"}
                    </div>
                  ) : (
                    <div>
                      {leftData?.staffRows?.[1]?.OPNO || "抓取中"}|{" "}
                      {leftData?.staffRows?.[1]?.StaffName2 || "抓取中"}
                    </div>
                  )}
                </div>
              </div>
              ...
              >
                {leftData?.todayCapacity_first_result} PCS
              </div>
              ...
              leftData?.selectedDayCapacity_first_result !== null ? (
                ...
                >
                  累積產能 :{leftData?.selectedDayCapacity_first_result} PCS
                </div>
              ) : (
              ...
              {shiftClass === "早班" ? (
                ...
                >
                  班別產能:
                  {leftData?.morningShiftCapacity_first_result} PCS
                </div>
              ) : (
                <div
                  className="Answer"
                ...
                >
                  班別產能:
                  {leftData?.nightShiftCapacity_first_result} PCS
                </div>
              )}
          ...
          <div className="MiddleContent">
            <div className="Content_Top">
              ...
              {machineOption === "注液機" ? (
                ...
                >
                  {Object.keys(group_Inject_01).map((groupName) => {
                    const labelMap = group_Inject_01[groupName]?.[0] || {};
                ...
                >
                  {Object.keys(group_Inject_02).map((groupName) => {
                    const labelMap = group_Inject_02[groupName]?.[0] || {};
                    const settingData = dataReference[groupName]?.[0] || {};
                    return (
                      <div key={groupName} style={{ marginBottom: "30px" }}>
                        <div
