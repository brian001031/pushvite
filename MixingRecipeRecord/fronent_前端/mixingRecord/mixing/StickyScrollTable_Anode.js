...
const StickyScrollTable = ({ 
  ...
  const localAnode = JSON.parse(localStorage.getItem("mixing_Anode")) || [];
  const [warningDataList, setWarningDataList] = useState([]); // 收集所有警告資料

  console.log("belowData:", belowData);
  ...
  // 使用 useMemo 避免每次 render 都重新创建验证规则物件
  const validationRules = useMemo(() => ({
    // N值设定
    Nvalue: {
      warning: { 
        min: typeof localAnode === "object" ? localAnode.Nvalue_Engineer_S : 0, 
        max: typeof localAnode === "object" ? localAnode.Nvalue_Engineer_E : 0,  
        message: 'N值必须在 ' + localAnode?.Nvalue_Engineer_S + '-' + localAnode?.Nvalue_Engineer_E + ' 之间 | Must between ' + localAnode?.Nvalue_Engineer_S + '-' + localAnode?.Nvalue_Engineer_E 
      },
      error: { min: 0, max: 1, message: "N值必须在 0-1 之間 | Must between 0-1" }
    },
    // 黏度设定
    Viscosity: {
      warning: {
        min: typeof localAnode === "object" ? localAnode.Viscosity_Engineer_S : 0, 
        max: typeof localAnode === "object" ? localAnode.Viscosity_Engineer_E : 0,  
        message: '黏度必须在 ' + localAnode?.Viscosity_Engineer_S + '-' + localAnode?.Viscosity_Engineer_E + ' 之间 | Must between ' + localAnode?.Viscosity_Engineer_S + '-' + localAnode?.Viscosity_Engineer_E 
      },
      error: { min: 1000, max: 7000, message: "黏度必须在 1000-7000 之間 | Must between 1000-7000" }
    },
    // 粒径设定
    ParticalSize: {
      warning: {
        min: typeof localAnode === "object" ? localAnode.ParticalSize_Engineer_S : 0,
        max: typeof localAnode === "object" ? localAnode.ParticalSize_Engineer_E : 0,  
        message: `粒径必须在 ${localAnode?.ParticalSize_Engineer_S}-${localAnode?.ParticalSize_Engineer_E} 之间 | Must between ${localAnode?.ParticalSize_Engineer_S}-${localAnode?.ParticalSize_Engineer_E}`
      },
    ...
    SolidContent: {
      warning: {
        min: typeof localAnode === "object" ? localAnode.SolidContent_Engineer_S : 0, 
        max: typeof localAnode === "object" ? localAnode.SolidContent_Engineer_E : 0,  
        message: `固含量必须在 ${localAnode?.SolidContent_Engineer_S}-${localAnode?.SolidContent_Engineer_E} 之间 | Must between ${localAnode?.SolidContent_Engineer_S}-${localAnode?.SolidContent_Engineer_E}` 
      },
      error: { min: 50, max: 70, message: "固含量必需在 50-70 之間 | Must between 50-70" }
    },
  }), [
    localAnode?.Nvalue_Engineer_S,
    localAnode?.Nvalue_Engineer_E,
    localAnode?.Viscosity_Engineer_S,
    localAnode?.Viscosity_Engineer_E,
    localAnode?.ParticalSize_Engineer_S,
    localAnode?.ParticalSize_Engineer_E,
    localAnode?.SolidContent_Engineer_S,
    localAnode?.SolidContent_Engineer_E,
  ]);
  ...
  const statusCheck = (id) => {
    ...
    if (currentErrors.length > 0 && collectWarningData && typeof collectWarningData === 'function') {
      console.log('=== 子組件 StickyScrollTable_Anode 發現錯誤 ===', currentErrors);
        ...
        >
          <thead>
            <tr>
              ...
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>Water-1 Loading Weight</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>Water-2 Loading Weight</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>Water-3 Loading Weight</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>NMP</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>Graphite1_1</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>Super_P_1</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>CMC_1</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>Graphite2_1</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>SBR_1</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>Graphite1_2</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>Super_P_2</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>CMC_2</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>Graphite2_2</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>SBR_2</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>NMP1_1</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>NMP1_2</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>PAA_1</th>
              <th style={{ whiteSpace: "nowrap", border: '1px solid #bbb' }}>PAA_2</th>
            </tr>
          </thead>
            ...
            {Array.isArray(belowData) && belowData.length > 0 ? (
              belowData.map((dataItem, index) => (
                <tr key={dataItem.id || index}>
                  ...
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.LotNo}</td>
                  {/* N 值提供 */}
                  <td style={{ border: '1px solid #bbb' }}>
                    <input
                      type="number"
                  ...
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.BatchStart ? moment(dataItem.BatchStart).format('YYYY-MM-DD HH:mm:ss') : ''}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.BatchEnd ? moment(dataItem.BatchEnd).format('YYYY-MM-DD HH:mm:ss') : ''}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.Filter_Mesh}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.TransportStart ? moment(dataItem.TransportStart).format('YYYY-MM-DD HH:mm:ss') : ''}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.TransportEnd ? moment(dataItem.TransportEnd).format('YYYY-MM-DD HH:mm:ss') : ''}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.Recipe}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.Water_1_LoadingWeight}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.Water_2_LoadingWeight}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.Water_3_LoadingWeight}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.NMP}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.Graphite1_1}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.Super_P_1}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.CMC_1}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.Graphite_2_1}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.SBR_1}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.Graphite1_2}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.Super_P_2}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.CMC_2}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.Graphite_2_2}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.SBR_2}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.NMP_1_1}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.NMP_1_2}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.PAA_1}</td>
                  <td style={{ border: '1px solid #bbb' }}>{dataItem.PAA_2}</td>
                </tr>
              ))
            ) : (
              <tr>
