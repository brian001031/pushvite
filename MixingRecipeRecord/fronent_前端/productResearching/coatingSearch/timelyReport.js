...
const TimelyReport = () =>{
    ...
    const fetchData = useCallback(async (page = nowPage) => {
        ...
        try{
            const response = await axios.get(
                `${config.apiBaseUrl}/coatingRecord/nowReport`,
                // `http://localhost:3009/coatingRecord/nowReport`,
                {
                    params: {
                        page: page,
                        pageSize: 20
            ...
            if (isMountedRef.current) {
                ...
                setData_Collect(response.data.data);
    ...
    const handleDataDisplay = (data_Collect, maxMembers) => {
        ...
        const rows = allData.map((item, idx) => (
            <tr key={idx}>
                <td style={{ minWidth: '100px' }}>{item.station || '-'}</td>
                <td style={{ minWidth: '150px' }}>{item.time || '-'}</td>
                <td style={{ minWidth: '100px' }}>{item.mixing_Batch || '-'}</td>
                <td style={{ minWidth: '120px' }}>{!isNaN(parseFloat(item.count)) ? parseFloat(item.count).toFixed(2) : '-'}</td>
                <td style={{ minWidth: '120px' }}>{!isNaN(parseFloat(item.faultyMeter_EmptySolder)) ? parseFloat(item.faultyMeter_EmptySolder).toFixed(2) : '-'}</td>
                <td style={{ minWidth: '120px' }}>{!isNaN(parseFloat(item.faultyMeter_Faulty)) ? parseFloat(item.faultyMeter_Faulty).toFixed(2) : '-'}</td>
                <td style={{ minWidth: '120px' }}>{!isNaN(parseFloat(item.faultyMeter_test)) ? parseFloat(item.faultyMeter_test).toFixed(2) : '-'}</td>
                <td style={{ minWidth: '100px' }}>{!isNaN(parseFloat(item.shiftMeter_percent)) ? parseFloat(item.shiftMeter_percent).toFixed(2)+ ' %' : '-'}</td>
                {/* <td style={{ minWidth: '100px' }}>{item.mixing_utilization !== undefined ? (item.mixing_utilization * 100).toFixed(2) + ' %' : '-'}</td> */}
            ...
            <Table striped bordered hover>
                <thead>
                    <tr>
                        ...
                        <th style={{ minWidth: '150px' }}>日期</th>
                        <th style={{ minWidth: '100px' }}>混漿批數</th>
                        <th style={{ minWidth: '120px' }}>塗佈生產米數</th>
                        ...
                        {Array.from({length: maxMembers}, (_, i) => <th key={i} style={{ minWidth: '100px' }}>作業人員{i+1}</th>)}
                    </tr>
                </thead>
                <tbody>
