...
router.get("/updatepage", async (req, res) => {
  ...
  try {
    ...
    // //正負極混漿目前realtable 無提供即時數量,先不計算
    // if (
    //   query_realtable.includes("mixing_realtime_c") ||
    //   query_realtable.includes("mixing_realtime_a")
    // ) {
    //   sql2 = `SELECT count(distinct ID) AS result FROM ${query_realtable} where 1 = 1 AND TIME BETWEEN '${startoem_dt}'  AND '${endoem_dt}'`;
    // }
...
router.get("/groupname_capacitynum", async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      if (query_realtable.includes("mixing_realtime_c")) {
        //正極混漿假數據生產數量
    ...
    console.log(
      "MYSQL->" + query_realtable.includes("mixing_realtime_c")
        ? "正極混漿站"
        : "負極混漿站" + "results = " + results
    );
