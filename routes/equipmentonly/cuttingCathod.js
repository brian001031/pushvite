...
router.get("/updatepage", async (req, res) => {
  ...
  sql = "SELECT * FROM mes.cutting_realtime_c ORDER BY ID DESC limit 1";
  ...
  switch (machineoptionConfirm) {
    ...
    case "報廢品":
      sql = `SELECT case WHEN SUM( Prdouction ) is NULL then '0' 
                ...
                OKNGSelection = '報廢品' AND Caseno like 'C%' AND
                Time between ? AND ?`;
      modifucation = "報廢品總計";
      break;
...
router.get("/fullmachinecapacity", async (req, res) => {
  ...
  //   console.log("正極模切班別產能SQL :", Cathnode_full_Pass_SQL);
  ...
  try {
    ...
    //計算班別產能
    const [cathnode_shift_range_amount] = await db2.query(
      Cathnode_full_Pass_SQL
    );

    const cathnode_dt_range_result = {};

    // 補齊沒出現的時間區間
    // shifts.map(([start, end], index) => {
    //   const dt_range_key = `${start}~${end}`;
    //   const map_dt_between = cathnode_shift_range_amount.find(
    ...
    //     phase = "今晚班";
    //   }
    //   const key = `正極模切站-${phase}總產能`;
    //   const total = map_dt_between ? map_dt_between.總計 : 0;
    //   cathnode_dt_range_result[key] = total;
    // });

    //另外方法
    // STEP 1: 將結果轉為 key-value 物件
    const map_dt_between = Object.fromEntries(
      cathnode_shift_range_amount.map((obj) => [obj.time_range, obj])
    );
    ...
    shifts.forEach(([start, end], index) => {
      ...
      if (!data) {
        console.warn(`正極模切站 ⚠️ 無資料 for 時段: ${key}，預設為 0`);
      }

      cathnode_dt_range_result[
        `正極模切站-${["昨晚班", "今早班", "今晚班"][index]}總產能`
      ] = total;
    ...
    // console.log(
    //   "cathnode_dt_range_result :",
    //   JSON.stringify(cathnode_dt_range_result, null, 2)
    // );

    res.status(200).json({
      data: rows[0],
      Total_capacity_shift: cathnode_dt_range_result,
    });
  } catch (error) {
    console.error("發生錯誤", error);
    res.status(500).json({
