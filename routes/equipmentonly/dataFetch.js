...
const DATA_FILE_PATH =
  process.env.DATA_FILE_PATH || path.join(__dirname, "data", "data.js");
...
async function readSpecificObjectFromFile(varName) {
  try {
    ...
    if (error.code === "ENOENT") {
      ...
      throw new Error(
        `資料檔案不存在: ${DATA_FILE_PATH}。請確保檔案已創建且路徑正確。`
      );
    }
    // 處理其他讀取檔案的錯誤
  ...
  try {
    ...
    if (currentData) {
      ...
      res
        .status(404)
        .json({
          message: `未找到變數 '${varName}' 的物件或檔案內容不符預期。`,
        });
    }
  } catch (error) {
    // 處理讀取檔案或內部錯誤
...
router.post("/data/update-data/:varName", async (req, res) => {
  ...
  try {
    ...
    if (!currentData) {
      ...
      console.warn(
        `檔案中未找到變數 '${varName}' 的物件，將從空物件開始更新或新增。`
      );
    }
    ...
    for (const key in currentData) {
      if (
        ...
        !Object.hasOwnProperty.call(incomingData, key)
      ) {
    ...
    if (changed) {
      ...
      res.json({
        message: `變數 '${varName}' 的資料已更新並寫入檔案`,
        updatedData: currentData,
      });
    } else {
      // 如果沒有任何變動，則不寫入檔案，直接返回訊息
      res.json({
        message: `變數 '${varName}' 的資料無變動`,
