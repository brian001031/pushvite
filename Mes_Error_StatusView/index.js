...
import api from "../MES_EquipmentProInfo_reBuild/api.js";
...
const Mes_Error_StatusView = () => {
  ...
  const sideOptionPathMap = {
    CoatingCathode: `${sideoption}/正極塗佈`,
    cutting_cathode: `${sideoption}/正極模切`,
    cutting_anode: `${sideoption}/負極模切`,
    stacking: `${sideoption}/疊片`,
    assembly: `${sideoption}/入殼`,
    oven: `${sideoption}/真空大小烘箱`,
    injection: `${sideoption}/注液`,
    chemosynthesis: `${sideoption}/化成`,
    capacity: `${sideoption}/分容`,
    ht_aging: `${sideoption}/高溫倉靜置`,
    rt_aging: `${sideoption}/常溫倉靜置`,
    edgeFolding: `${sideoption}/精封`,
    sulting: `${sideoption}/分選判別`,
    mixingAnode: `${sideoption}/負極混漿`,
    mixingCathode: `${sideoption}/正極混漿`,
  };
  ...
  const messide_error_callallapi = (sideoption) => {
    // 分選判別站
    if (String(sideoption).includes("sulting")) {
      return api.callSulting_Abnormality_errorinfo(
        select_error,
        runlogDate,
        sideoption
      );
  ...
  const fetchError_fromMesDB_or_Log = async () => {
    ...
    try {
      const api_error_info = await messide_error_callallapi(sideoption);

      if (api_error_info === null || typeof api_error_info === "undefined") {
        toast.error(
          `找不到對應的 API，請確認${sideoption}站 api 設定是否正確或無建立`
        );
        return;
      }

      // console.log("回傳異常資訊:", JSON.stringify(api_error_info, null, 2));

      if (!api_error_info?.allinfo?.length) {
      ...
      // 將字串日期轉成 Date 物件陣列
      const error_record_Dates = api_error_info.log_date
        .map((d) => new Date(d))
        .filter((d) => !isNaN(d));

      // console.log("轉換後可用日期陣列 =", error_record_Dates);

      setmes_error_data(api_error_info.allinfo || []);
  ...
  const getTitle_item_color = (count) => {
    if (count <= 5) return "#a6f136ff"; // low（綠）
    if (count > 5 && count <= 10) return "#a9c3f4ff"; // middle（黃）
    return "#e86a6aff"; // high（紅）
  };
  ...
  return (
    <div className="mes_error_statusview">
      ...
      >
        MES:{sideOptionPathMap[sideoption]}站異常狀態查詢
      </div>
      <br />
      {/* 換行區塊 */}
      ...
      <Container className="py-4">
        ...
        <Row className="g-3">
          {currentDates.slice(0, 6).map((group, idx) => {
            ...
            console.log("目前日期 =", group.date, "頁碼 =", pageIndex);
            const currentItems = group.pages[pageIndex] || [];
            // 計算總筆數，安全寫法
            const totalItems = group.pages.reduce(
              (sum, page) => sum + (Array.isArray(page) ? page.length : 0),
              0
            );

            const variant = getTitle_item_color(totalItems);
            const titleTextColor =
              totalItems > 10 ? "text-red-100" : "text-black";

            return (
              <Col key={group.date} md={4}>
                <Card className="h-100">
                  <Card.Body>
                    {/* 日期標題 */}　 {/*  Title 外框加背景色 + 內距 */}
                    <div
                      className="p-2 mb-3 rounded"
                      style={{
                        backgroundColor: variant, // ← 直接使用 HEX
                        color: titleTextColor, // ← 給字體 HEX 或 black/white
                      }}
                    >
                      <Card.Title className="mb-0">
                        {group.date ?? "未知日期"}（{totalItems}筆）
                      </Card.Title>
                    </div>
                    {/* 錯誤列表 */}
                    <div>
                    ...
                    {group.totalPages > 1 && (
                      <div className="d-flex justify-content-between align-items-center mt-3">
                        ...
                        <Button
                          ...
                          disabled={group.pageIndex === group.totalPages - 1}
                        >
                          後一清單
                        </Button>
                      </div>
      ...
      {/* 點擊頁碼 */}
      <div className="w-full flex justify-center mt-4 space-x-2 flex-wrap">
        {Array.from({ length: totalPages }).map((_, idx) => {
          const page = idx + 1;
          return (
            <button
